...
/*
 * RX queue 是 single-producer/single-consumer ring buffer：USB callback 寫 head，
 * 主迴圈讀 tail。滿載時設 overflow，主迴圈稍後回報錯誤而不阻塞 callback。
 */
...
/**
  ...
  * @note MOTOR_CAN_STATUS_OK 由 ReportStartStatus() 直接處理，不需要文字 mapping。
  */
...
/**
  ...
  * @param value 成功時接收 int32_t 結果。
  * @retval 1 表示完整 token 落在 INT32_MIN..INT32_MAX；否則為 0。
  */
static uint8_t USB_Command_ParseInt32(const char *text, int32_t *value)
{
  char *end;
  long parsed;
  ...
  /* HOME offset 允許負值，但必須完整落在 int32_t 範圍內。 */
  errno = 0;
...
static void USB_Command_ExecuteLine(char *line)
{
  ...
  if (strcmp(command, "HELP") == 0)
  {
    ...
    USB_Command_QueueText("HOME <bus> <id> <FWD|REV> <high_rpm> <low_rpm> <offset> <timeout_ms> CONFIRM");
  ...
  if (strcmp(command, "STATUS") == 0)
  {
    ...
    USB_Command_QueueText(
                           "OK STATUS ems=%s operation=%s can1=%uK can2=%uK ems_resume_without_reset=1",
  ...
  if (strcmp(command, "HOME") == 0)
  {
    ...
    if ((token_direction != NULL) &&
        (strcmp(token_direction, "FWD") == 0))
    {
      direction_value = 0U;
    }
    ...
    if ((!USB_Command_ParseNumber(token_bus, 2U, &bus_value)) ||
        ...
        (!USB_Command_ParseNumber(token_high_speed, 3000U,
                                  &high_speed_value)) ||
        (high_speed_value < 1U) ||
        (!USB_Command_ParseNumber(token_low_speed, 100U,
                                  &low_speed_value)) ||
        (low_speed_value < 1U) ||
        (low_speed_value > high_speed_value) ||
        (!USB_Command_ParseInt32(token_offset, &offset_value)) ||
    ...
    status = MotorCAN_StartHome((uint8_t)bus_value, id_value,
                                direction_value, high_speed_value,
                                low_speed_value, offset_value, timeout_value);
...
/**
  ...
  * @retval 無。
  * @note 會處理 INFO、SET_ID、PROVISION_1M、TEST、HOME 與一般 CAN 錯誤。
  */
  ...
  while (MotorCAN_GetEvent(&event))
  {
    switch (event.type)
    {
      ...
      case MOTOR_CAN_EVENT_TEST_STOPPED_BY_EMS:
        USB_Command_QueueText(
          "ERR TEST EMS_ACTIVE bus=%u id=0x%03X control_stop_sent=1",
          event.bus, event.id);
        break;

      case MOTOR_CAN_EVENT_HOME_FINISHED:
        USB_Command_QueueText(
          "OK HOME bus=%u id=0x%03X complete enabled=1",
          event.bus, event.id);
        break;
...
/**
  ...
  * @retval 無。
  * @note EMS blocked 時暫停 RX parser，但仍會回報既有事件並送完 TX queue。
  */
void USB_Command_Process(void)
{
  /*
   * EMS blocked 時不解析新命令，但仍排出既有 MotorCAN 事件與 TX 回覆，
   * 讓上位機能收到停止／錯誤原因。釋放清理完成後才恢復 RX parser。
   */
...
/**
  ...
  * @retval 無。
  * @note 在 USB callback context 執行；EMS blocked 時丟棄整包，queue 滿時設 overflow。
  */
void USB_Command_OnReceive(const uint8_t *data, uint32_t length)
{
  ...
  /* EMS 期間直接丟棄新輸入，避免釋放後執行先前排隊的運動命令。 */
  if (EMS_AreCommandsBlocked())
  {
    return;
  }
